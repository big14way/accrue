...
cfg = {
  ...
  "voice": "Samantha", "voice_rate": 150, "voice_tempo": 0.9, "crossfade": 0.5,
  ...
  "scenes": [
    ...
    lap("s1", "landing.webm", "01", "Live on Monad testnet, settling in Agora AUSD", "Job #1 is funded with 1,000 AUSD. The counter moves every 400 ms block.",
        "This is Accrue live on Monad testnet, settling in Agora's AUSD. Job number one is funded with one thousand AUSD, and the yield counter moves with every four hundred millisecond block, because interest is accrued per block.", cut=[4.0, round(dur("landing.webm") - 0.3, 2)], max_speed=1.6),
    lap("s2", "post.webm", "02", "Post a job in plain English", "Deadline, freshness rule and the provider's completion bonus are committed on chain.",
        "A client describes the work in plain English, sets the deadline, the freshness rule, and how much of the yield the provider earns as a completion bonus. Funding is one transaction, and the budget is inside the vault before the next block.", cut=[0.8, round(dur("post.webm") - 0.3, 2)], max_speed=1.4),
    lap("s3", "job.webm", "03", "Yield accrues per block, anchored to the contract", "previewSettlement() is re-read every two seconds; the timeline comes from Envio.",
        "Every job page shows the yield accruing in real time, anchored to the contract's own settlement preview, with the full timeline indexed by Envio.", cut=[3.5, round(dur("job.webm") - 0.3, 2)], max_speed=1.4),
    ...
    lap("s6", "term_d2.webm", "06", "One transaction settles every leg", "Drill 2 on the AUSD deployment: vault redeemed, pool repaid, provider, client and protocol paid, ERC-8004 written.",
        "When delivery is verified, one transaction does everything. The vault is redeemed. The pool is repaid first. The provider receives the principal plus its share of the yield. The client and the protocol receive theirs. And the outcome is written to ERC-8004 in the same block.", cut=[0.2, round(dur("term_d2.webm") - 0.3, 2)], max_speed=1.3),
    ...
    lap("s7", "refusals.webm", "07", "Refusals you can link to", "Stale data, missed deadlines, empty hashes and wrong-hash verdicts revert in the caller's own transaction.",
        "Deterministic rules revert in the provider's own transaction: stale data, a missed deadline, an empty hash, a verdict for the wrong hash. Every refusal is a transaction you can link to.", cut=[5.0, round(dur("refusals.webm") - 0.3, 2)], max_speed=1.4),
    lap("s8", "term_d6.webm", "08", "A hostile Claude agent tried to get paid without delivering", "Drill 6 transcript: direct completion, fabricated deliverable and self-attestation all refused on chain.",
        "We handed a Claude agent the provider key and told it to get paid without doing the work. It tried a direct completion, a fabricated deliverable and a self-attestation. Every path was refused on chain. The advance it took became a recorded default, and its credit limit fell from fifty-five to forty percent.", cut=[0.2, round(dur("term_d6.webm") - 0.3, 2)], max_speed=1.6),

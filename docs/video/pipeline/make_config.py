...
cfg = {
  ...
  "voice": "en-US-AndrewMultilingualNeural", "voice_rate": "+16%", "voice_tempo": 1.0, "crossfade": 0.6,
  ...
  "scenes": [
    ...
    {"id": "p1", "kind": "slide", "image": f"{IMG}/vault.jpg", "label": "The problem",
     ...
     "vo": "Meet a price-feed agent on Monad. It just delivered a week of hourly reports to a trading agent. Its budget sat idle in escrow all week. It waits for settlement because someone must approve. It cannot borrow against finished work, because nobody can price its credit. And a server decides the verdict."},
    ...
    {"id": "p3", "kind": "slide", "diagram": "flow", "label": "What Accrue changes",
     ...
     "vo": "Accrue changes all three. The budget earns yield from the block it is funded. The provider borrows against the job today, priced from its own on-chain record. The verdict comes from a contract fed by Chainlink, not a server. Immutable contracts, no admin key."},
    ...
    lap("s1", "landing.webm", "01", "Live on Monad testnet, settling in Agora AUSD", "Job #1 holds 1,000 AUSD. The counter moves with every 400 ms block.",
        "Live on Monad testnet, settling in Agora's AUSD. Job one holds one thousand AUSD, and its yield counter moves with every four hundred millisecond block.", cut=[4.0, 28.0], max_speed=2.0),
    lap("s2", "post.webm", "02", "Post a job in plain English", "Deadline, freshness rule and the provider's completion bonus are committed on chain.",
        "A client describes the work in plain English, sets the deadline and the completion bonus, and funds it. The budget is in the vault before the next block, and every history view comes from Envio HyperIndex on Envio Cloud.", cut=[0.8, round(dur("post.webm") - 0.3, 2)], max_speed=2.0),
    ...
    lap("s5", "pool.webm", "05", "An advance, a bad delivery, a priced loss", "Drill 3: bond seized, shortfall recorded, ERC-8004 default written, limit 10 % to 0 %.",
        "Drill three: an advance, a mismatching hash, a rejection. Bond seized, shortfall recorded, a default written to its reputation, limit cut from ten percent to zero. Risk priced, loss recorded.", cut=[7.2, round(dur("pool.webm") - 0.3, 2)], max_speed=2.2),
    lap("s6", "term_d2.webm", "06", "One transaction settles every leg", "Drill 2 on the AUSD deployment: vault redeemed, pool repaid, provider, client and protocol paid, ERC-8004 written.",
        "Verified delivery settles in one transaction: vault redeemed, pool repaid first, provider paid with its yield bonus, client and protocol paid, outcome written to its reputation.", cut=[0.2, round(dur("term_d2.webm") - 0.3, 2)], max_speed=2.2),
    lap("s6b", "explorer_still.mp4", "", "The settlement on the Monad explorer", "One Attest call: 1,000 AUSD out of the vault, 1,000 to the provider, the client's and the protocol's share of the yield.",
        "On the explorer, one call: a thousand AUSD out of the vault, a thousand to the provider, every yield share.", cut=[0, 7.0]),
    ...
    {"id": "p4", "kind": "slide", "image": f"{IMG}/speed.jpg", "label": "Why Monad", "columns": 2,
     ...
     "vo": "Why Monad: per-block interest at four hundred milliseconds, and refusals you can link to, for cents. Drill five settled twenty jobs back to back, about a fifth of a MON per job. And the agent registries live here, so credit history is native."},
    ...
    {"id": "p5", "kind": "slide", "image": f"{IMG}/nodes.jpg", "label": "Built on", "columns": 2,
     ...
     "vo": "Every integration is load-bearing: Agora AUSD, Chainlink CRE, Envio, ERC-8004, x402 and MPP, Privy, Nansen, and Morpho on mainnet."},
    ...
    {"id": "p6", "kind": "slide", "image": f"{IMG}/ribbons.jpg", "label": "Revenue model and traction",
     ...
     "vo": "Revenue scales with escrowed volume: a protocol share of realised yield on every settled job, ten percent in the demo, advance interest for the pool, then a spread and an origination fee on mainnet. Forty-eight jobs have settled across two testnet deployments, every step linked on chain."},

...
cfg = {
  ...
  "scenes": [
    ...
    lap("s7", "refusals.webm", "07", "Refusals you can link to", "Stale data, missed deadlines, empty hashes and wrong-hash verdicts revert in the caller's own transaction.",
        "Deterministic rules revert in the provider's own transaction: stale data, a missed deadline, an empty hash, a verdict for the wrong hash. Every refusal is a transaction you can link to.", cut=[5.0, round(dur("refusals.webm") - 0.3, 2)], max_speed=1.6),
